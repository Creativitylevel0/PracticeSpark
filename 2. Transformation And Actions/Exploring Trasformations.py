...
qdf = (fire_df.selectExpr("count(distinct CallType) as distinct_call_type_count"))
display(qdf)
...
df = (fire_df.select("CallType")
            .where("CallType is not null")
            .withColumnRenamed("CallType","distinct_call_type")
            .distinct()
            )
...
(fire_df.selectExpr( "sum(NumAlarms)", "avg(Delay)", "min(Delay)", "max(Delay)").show())
...
count = fire_df.select("CallDate").distinct().count()
print(count)
